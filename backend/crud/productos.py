...
from utils.generar_codeBar import BarcodeGenerator  # Generador de código de barras.
...
def create_producto(db: Session, producto: ProductoCreate):
    # Verificar si el código de barras está definido, si no, generarlo automáticamente
    
    generador = BarcodeGenerator()
    codigo = generador.generate(producto.codigo_barras)# Genera el código de barras.
    producto.codigo_barras = codigo  # Asigna el código de barras generado al producto
    
    
    
    # Crear el objeto del producto con las fechas de creación y actualización
    db_producto = Producto(**producto.dict(), fecha_creacion=datetime.now(), fecha_actualizacion=datetime.now())
    db.add(db_producto)  # Añadir el producto a la sesión
    db.commit()  # Guardar los cambios en la base de datos
    db.refresh(db_producto)  # Refrescar para obtener los datos generados (e.g., ID)
    return db_producto
