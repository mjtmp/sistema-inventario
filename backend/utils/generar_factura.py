...
'''def obtener_numero_factura(db: Session):
    factura = db.query(Factura).order_by(Factura.factura_id.desc()).first()
    if factura:
        ultimo_numero = int(factura.numero_factura.replace('FACT', ''))
        nuevo_numero = ultimo_numero + 1
    else:
        nuevo_numero = 1
    return f"FACT{nuevo_numero:03d}
...
def generar_factura(cliente, factura_info, productos):
    ...
    pdf.factura_info(factura_info['numero'], date.today().isoformat())
...
'''def obtener_numero_factura(db: Session):
    factura = db.query(Factura).order_by(Factura.numero_factura.desc()).first()
    if factura and factura.numero_factura.isdigit():  # Verifica que es numérico
        return int(factura.numero_factura) + 1  # Convierte a entero y suma 1
    else:
        return 1  # Si no hay ninguna factura, empieza en 1'
    
    factura = db.query(Factura).order_by(Factura.factura_id.desc()).first()
    if factura:
        return factura.numero_factura + 1
    return 1'''
...
from models.models import Factura  # Asegúrate de importar el modelo Factura
...
class PDF(FPDF):
    ...
    def factura_info(self, numero_factura, fecha):
        self.set_xy(130, 60)
        self.set_font('Arial', 'B', 10)
        self.cell(0, 5, f'Factura Nº: {numero_factura}', 0, 1, 'R')
        self.cell(0, 5, f'Fecha: {fecha}', 0, 1, 'R')
        self.ln(10)
    ...
    def productos_table(self, productos):
        ...
        for prod in productos:
            # Accedemos a los atributos con la notación de punto
            cantidad = prod.cantidad
            descripcion = prod.descripcion
            precio_unitario = prod.precio_unitario
            tiene_iva = prod.tiene_iva
